...
{
    // Produce:
    //   const Type& Name   â€” const + ref
    //   const Type  Name   â€” const only
    //   Type&       Name   â€” ref only
    //   Type        Name   â€” neither
...
FString FCodeForgePropertyDef::BuildSpecifierString() const
{
    ...
    // Visibility/Edit specifiers â€” mutually exclusive, first true wins
    if (bVisibleAnywhere)
    {
    ...
    // Blueprint specifiers â€” mutually exclusive, first true wins
    ...
    // Replication â€” RepNotify takes priority over plain Replicated
    if (bReplicated)
    {
        if (bRepNotify)
        {
...
FString FCodeForgeFunctionDef::BuildSpecifierString() const
{
    ...
    // Exec is a standalone specifier â€” not combined with BlueprintCallable
    if (bExec)
    {
        Specifiers.Add(TEXT("Exec"));
    }
    // BlueprintPure implies BlueprintCallable â€” treat as mutually exclusive
...
FString UCodeForgeBlueprint::GetClassSpecifiers() const
{
    ...
    // Note: Replication is NOT a UCLASS specifier â€” it is enabled via
...
// ---------------------------------------------------------------------------
// UCodeForgeBlueprint â€” validation helpers
// ---------------------------------------------------------------------------
...
namespace CodeForgeValidation
{
    ...
    static const TMap<FString, FString>& GetTypeCorrectionMap()
    {
        ...
        if (Map.Num() == 0)
        {
            // C++ primitives (wrong case â†’ correct)
            ...
            // UE5 types (wrong case â†’ correct)
     ...
     *  - Empty type
     *  - Common miscapitalization (e.g. "Float" â†’ "float")
...
// ---------------------------------------------------------------------------
// UCodeForgeBlueprint â€” Validate()
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// UCodeForgeBlueprint â€” ApplyAutoFix()
// ---------------------------------------------------------------------------
...
void UCodeForgeBlueprint::ApplyAutoFix(const FString& AutoFixId)
{
    ...
    {
        // Format: "FixClassPrefix:A" â€” prepend the correct prefix character
