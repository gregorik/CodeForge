...
// Implementation of FCodeForgeTemplateEngine â€” a minimal, dependency-free
    ...
    int32& OutEnd) const
{
    ...
    while (Pos < ContentLen)
    {
        ...
        {
            // Make sure this is actually a fresh occurrence â€” advance past it
            ++Depth;
            Pos += OpenTag.Len();
            continue;
        }
...
// ---------------------------------------------------------------------------
// ProcessInternal â€” the core recursive processing method
// ---------------------------------------------------------------------------
    ...
    int32 LineOffset) const
{
    ...
    if (TemplateLen == 0)
    {
        return Result; // empty template â€” success with empty output
    }
    ...
    while (Pos < TemplateLen)
    {
        ...
        {
            // No more directives â€” append the rest as-is
        ...
        if (Directive.StartsWith(TEXT("#each ")))
        {
            ...
            // Look up the array â€” absence means empty (not an error)
        ...
        // ------------------------------------------------------------------
        // {{/if}} or {{/each}} â€” unexpected closing tag (no matching open)
        // ------------------------------------------------------------------
        ...
        // ------------------------------------------------------------------
        // {{VariableName}} â€” variable substitution
        // ------------------------------------------------------------------
        {
