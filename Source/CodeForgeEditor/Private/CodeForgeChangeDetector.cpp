...
// Implements FCodeForgeChangeDetector — loads, saves, and compares the
...
void FCodeForgeChangeDetector::LoadManifest(const FString& ProjectDir)
{
	...
	{
		// No manifest yet — first run.  Entries stays empty.
		return;
	}
	...
	const FString& ClassName, const FCodeForgeManifestEntry& NewEntry) const
{
	...
	if (!Existing)
	{
		// First generation — always structural
		return true;
	}
	...
	// No structural differences detected — behavioral only
	return false;
}
