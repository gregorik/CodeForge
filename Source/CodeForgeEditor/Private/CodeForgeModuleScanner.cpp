...
// Implements FCodeForgeModuleScanner — discovers project modules by reading
...
TArray<FCodeForgeModuleInfo> FCodeForgeModuleScanner::ScanProject(const FString& ProjectDir) const
{
	...
	for (const FString& ModuleName : DeclaredModuleNames)
	{
		...
		// Determine Public/Private dirs — fall back to SourceDir if they don't exist on disk
