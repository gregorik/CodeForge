...
// Implements FCodeForgeGenerator â€” the orchestration layer that reads a
...
FCodeForgeTemplateContext FCodeForgeGenerator::BuildContext(const UCodeForgeBlueprint* Blueprint) const
{
    ...
    case ECodeForgeBlueprintKind::Class:
    {
        ...
        for (const FCodeForgeFunctionDef& Func : Blueprint->Functions)
        {
            ...
            {
                // Non-NativeEvent with a body â†’ emit implementation in source file
    ...
    case ECodeForgeBlueprintKind::Enum:
    {
        ...
        for (const FCodeForgeEnumEntryDef& Entry : Blueprint->EnumEntries)
        {
            ...
            // Build complete UMETA suffix â€” empty string when no meta is set
    ...
    const UCodeForgeBlueprint* Blueprint, bool bHeader) const
{
    ...
    case ECodeForgeBlueprintKind::Enum:
        // Enums have a header only â€” no source file
...
FString FCodeForgeGenerator::LoadTemplate(const FString& TemplateName) const
{
    ...
    if (SearchRoot.IsEmpty())
    {
        // Default: plugin Content/Templates directory â€” use IPluginManager for robust path resolution
    ...
    // Sub-directories to search (order matters â€” prefer exact match in common first)
    ...
    // Not found in any sub-directory â€” return empty (engine will error)
    return TEXT("");
}
